...
async function applyStock(pool, itemType, qty, meters, weight, bags, sign) {
    if (itemType === "drip") {
        await pool.query(
            `UPDATE stock SET
                quantity   = quantity + $1,
...
router.post("/", async (req, res) => {
    try {
        const { item_type, worker, shift, date, quantity, meters, weight, bags, notes } = req.body;
        ...
        const result = await pool.query(
            `INSERT INTO production (item_type, worker, shift, date, quantity, meters, weight, bags, notes)
             VALUES ($1, $2, $3, COALESCE($4::DATE, CURRENT_DATE), $5, $6, $7, $8, $9)
             RETURNING *`,
            [item_type, worker || null, shift || "day", date || null, qty, mts, wt, bg, notes || null]
        );

        await applyStock(pool, item_type, qty, mts, wt, bg, +1);
...
router.delete("/:id", async (req, res) => {
    try {
        ...
        await applyStock(pool, p.item_type,
            parseFloat(p.quantity), parseFloat(p.meters), parseFloat(p.weight), parseFloat(p.bags), -1);
